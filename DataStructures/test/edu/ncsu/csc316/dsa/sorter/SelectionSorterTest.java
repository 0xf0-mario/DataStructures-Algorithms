...
/**
 * SelectionSortTest class will be used to ensure that the elements are in the desired ascending order
 * as defined by their own natural order
 * @author Mario Medel
 *
 */
public class SelectionSorterTest {
	/** sorts students based on ID */
	SelectionSorter<Student> studentIdSort;
	/** sorts students based on GPA */
	SelectionSorter<Student> studentGPASort;
	
	/**
	 * used before every test case and we will reuse these objects during the testing
	 * @throws java.lang.Exception if the object cannot be created
	 */
	@Before
	public void setUp() throws Exception {
		StudentIDComparator idCompare = new StudentIDComparator();
		studentIdSort = new SelectionSorter<Student>(idCompare);
		
		StudentGPAComparator gpaCompare = new StudentGPAComparator();
		studentGPASort = new SelectionSorter<Student>(gpaCompare);
		
	}

	/**
	 * Test method for {@link edu.ncsu.csc316.dsa.sorter.SelectionSorter#SelectionSorter(java.util.Comparator)}.
	 */
	@Test
