...
public class AdjacencyMatrixGraph<V, E> extends AbstractGraph<V, E> {
	/** n by m matrix used to represent edges between two vertices n_i and m_j */
    ...
    public void removeVertex(Vertex<V> vertex) {
        ...
        for(int i = 0; i < matrix.length; i++) {
        	...
        	if(isDirected()) {
        		e2 = matrix[i][vertIdx];
        		if(e2 != null)
        			edgeList.remove(e2.getPosition());
        	}
        	if(e1 != null)
        		edgeList.remove(e1.getPosition());
        }
        int newLen = matrix.length - 1;
        @SuppressWarnings("unchecked")
		GraphEdge[][] shifted = (GraphEdge[][]) (new AbstractGraph.GraphEdge[newLen][newLen]);
        if(vertIdx == newLen) {
        	...
        	for(int i = 0; i < matrix.length; i++) {
        		for(int j = vertIdx + 1; j < matrix.length; j++) {
    ...
    public void removeEdge(Edge<E> edge) {
        ...
        matrix[validate(ends[0]).getIndex()][validate(ends[1]).getIndex()] = null;
        
        if(!isDirected()) {
        	matrix[validate(ends[1]).getIndex()][validate(ends[1]).getIndex()] = null;
        }
     ...
     * 
     * @param v vertex to cast into a MatrixVertex
