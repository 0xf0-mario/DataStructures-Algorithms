...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	/**
	 * references the Balanceable Binary tree object and uses its operations to simplify our operations
	 */
     ...
     * @param compare will be used to specify our specific ordering of elements
     */
    ...
    private Position<Entry<K, V>> treeMin(Position<Entry<K, V>> p) {
        ...
        while (isInternal(current)) { current = left(current); }
        return parent(current); //reached a sentinel node so we return the parent
    }
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public void rotate(Position<Entry<K, V>> p) {
            //there will be three nodes involved in the rotation (including p)
        	...
        	//check to see if single rotation
        	if(grandParent == null) {
        		setRoot(node);
        		node.setParent(null);
        	} else {
        		//line the node as a child of the grandparent
        	...
        	//link the parent and node and transfer node's subtree always
        ...
        protected static class BSTNode<E> extends BinaryTreeNode<E> {
        	/**
        	 * depending on the type of tree we will store a different property.
        	 * AVL trees will define property as the height of each individual node
        	 * Red-Black trees will define the property as the color of the node
        	 */
     ...
     * @return the root of the THIS tree
     */
    public Position<Entry<K, V>> root() {
        return tree.root();
    }
    /**
     * Accesses and returns the parent of the provided node position
     * @param p node position to check for a parent
     * @return parent of the p node
     */
    public Position<Entry<K, V>> parent(Position<Entry<K, V>> p) {
        return tree.parent(p);
    }
    /**
     * checks to see if the provided node is an internal node (1 or 2 children)
     * @param p node position to check if it is internal to the tree
    ...
    /**
     * sets the provided position to the entry
     * @param p node to update with a new entry
     * @param entry replacement entry for the position of p
     * @return entry which was replaced
     */
    public Entry<K, V> set(Position<Entry<K, V>> p, Entry<K, V> entry) {
        return tree.set(p, entry);
    }
    /**
     * checks to see if the provided node is a leaf (no children)
     * @param p node to check if it has no children
     * @return true if the node has 0 children
     */
    public boolean isLeaf(Position<Entry<K, V>> p) {
        return tree.isLeaf(p);
    }
    /**
     * checks to see if the provided node is a root
     * @param p	node to check if it is a root (no parents)
    ...
    /**
     * checks the left child of the provided node position
     * @param p node to check for the left child
     * @return left child of node p
     */
    public Position<Entry<K, V>> left(Position<Entry<K, V>> p) {
        return tree.left(p);
    }
    /**
     * checks for the right child of the provided node position
     * @param p node to check for the right child
     * @return right child of node p
     */
    public Position<Entry<K, V>> right(Position<Entry<K, V>> p) {
        return tree.right(p);
    }
    /**
     * retrieves the sibling(s) of node p, if it has one (or many).
     * @param p node to retrieve its sibling(s)
     * @return  sibling(s) of this node p
     */
