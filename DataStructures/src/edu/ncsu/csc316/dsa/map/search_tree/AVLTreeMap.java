...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
     ...
     * @param compare defines the natural order to compare the keys
     */
    ...
    private void rebalance(Position<Entry<K, V>> p) {
        //track the height of the node x
    	...
    	do {
    		...
    		if(!isBalanced(x)) {
    			...
    			//find the grand child with the "taller" height
    			Position<Entry<K, V>> grandChild = tallerChild(child);
    			//perform Tri Node Restructuring at the grand child
    ...
    private Position<Entry<K, V>> tallerChild(Position<Entry<K, V>> p) {
    	...
    	if ( isRoot(p) )
    	   return left(p);
//    	x is left child of parent(x) then
    	if ( p == left(parent(p)))
    	   return left(p);
    	else
    	   return right(p);
